...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateCropBox() {
    ...
    if (cropBox && dimensionsDisplay) {
      ...
      dimensionsDisplay.textContent = `${Math.round(cropData.width)} × ${Math.round(cropData.height)} px`;
    }
  }
  ...
  function setupCropEvents(cropBox, wrapper) {
    ...
    document.addEventListener('mousemove', function(e) {
      ...
      if (isDragging) {
        cropData.x = Math.max(0, Math.min(imgWidth - cropData.width, cropData.x + deltaX));
        cropData.y = Math.max(0, Math.min(imgHeight - cropData.height, cropData.y + deltaY));
      } else if (isResizing) {
