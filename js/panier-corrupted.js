...
window.ajouterAuPanierProduit = function(produit) {
  ...
  /**
   * Synchronise le panier localStorage avec le panier session
   */
  async synchroniserAvecSession() {
    try {
      // Envoyer le panier localStorage au serveur pour synchronisation
      const currentPath = window.location.pathname;
      let panierUrl = '/pages/panier.php';
      
      // Ajuster le chemin selon la page actuelle  
      if (currentPath.includes('/pages/') || currentPath.includes('/admin/') || currentPath.includes('/clients/') || currentPath.includes('/formulaires/') || currentPath.includes('/pages-perso/')) {
        panierUrl = '../pages/panier.php';
      }
      
      const response = await fetch(panierUrl, {
        method: 'POST',
        headers: {
        ...
        body: new URLSearchParams({
          action: 'sync_panier',
          panier_client: JSON.stringify(this.panier)
        })
      });

      if (response.ok) {
        const data = await response.json();
        if (data.success && data.panier_session) {
          // Convertir les éléments de session au format localStorage pour une synchronisation complète
          const panierFusionne = [];
          
          // Ajouter les éléments du localStorage existants
          this.panier.forEach(item => {
            panierFusionne.push(item);
          });

          // Ajouter les éléments de session (photo/perso) s'ils ne sont pas déjà présents
          data.panier_session.forEach(sessionItem => {
            const existeDejaIndex = panierFusionne.findIndex(item => item.id === sessionItem.id);
            if (existeDejaIndex === -1) {
              // Convertir au format localStorage
              panierFusionne.push({
                id: sessionItem.id,
                quantite: sessionItem.quantite,
                prix: sessionItem.prix,
                details: sessionItem.details,
                dateAjout: sessionItem.dateAjout || new Date().toISOString(),
                fromSession: true // Marquer comme venant de session
              });
            }
          });

          this.panier = panierFusionne;
          this.sauvegarderPanier();
          console.log('[DEBUG] Synchronisation terminée, panier fusionné:', this.panier.length, 'articles');
        }
      }
    } catch (error) {
...
function supprimerDuPanier(produitId) {
  ...
  const index = panierManager.panier.findIndex((item) => item.id === produitId);
  console.log('[DEBUG] Index trouvé:', index, 'sur', panierManager.panier.length, 'articles');
  
  if (index > -1) {
    // Supprimer seulement l'élément à l'index trouvé
    const itemSupprime = panierManager.panier.splice(index, 1);
    console.log('[DEBUG] Article supprimé:', itemSupprime[0]?.details?.designation);
    console.log('[DEBUG] Panier après suppression:', panierManager.panier.length, 'articles');
